...
public class AudioController : SingletonMonoBehaviour<AudioController>
{
    ...
    public AudioObject PlayAudioItem(AudioItem sndItem, float volume, Vector3 worldPosition, Transform parentObj = null, float delay = 0, float startTime = 0, bool playWithoutAudioObject = false, AudioObject useExistingAudioObj = null, double dspTime = 0)
    {
        ...
        foreach (var sndSubItem in sndSubItems)
        {
            if (sndSubItem != null)
            {
                if (sndSubItem.SubItemType == AudioSubItemType.Item && sndSubItem.ItemModeAudioID == sndItem.Name)
                {
                    Debug.LogError("不可选择自己作为SubItem!!!!");
                    continue;
                }
    ...
    {
        double newSystemTime = Time.realtimeSinceStartup; //SystemTime.timeSinceLaunch;
    ...
    public AudioObject PlayAudioSubItem(AudioSubItem subItem, float volume, Vector3 worldPosition, Transform parentObj, float delay, float startTime, bool playWithoutAudioObject, AudioObject useExistingAudioObj, double dspTime = 0)
    {
        ...
        //安全措施，AudioSource.time超出音频长度会报错
        var trueStartTime = startTime + subItem.SafeClipStartTime;
        if (subItem.ClipStopTime > 0)
        {
            if (trueStartTime > subItem.ClipStopTime)
            {
                trueStartTime = subItem.ClipStopTime * 0.9f;
            }
        }
        else
        {
            if (trueStartTime > subItem.Clip.length)
            {
                trueStartTime = subItem.Clip.length * 0.9f;
            }
        }
        sndObj.primaryAudioSource.time = trueStartTime;
        ...
        logData.delay = (float)dspTime + delay + subItem.Delay + audioItem.Delay;
        logData.parentObject = parentObj != null ? parentObj.name : "";
        logData.position = worldPosition;
        logData.startTime = startTime + subItem.SafeClipStartTime;
        logData.volume = volumeWithCategory;

#if UNITY_AUDIO_FEATURES_4_1
        if (dspTime > 0)
        {
            logData.scheduledDspTime = Time.time + (float)((dspTime + delay + subItem.Delay + audioItem.Delay) - AudioSettings.dspTime);
        }
#endif
